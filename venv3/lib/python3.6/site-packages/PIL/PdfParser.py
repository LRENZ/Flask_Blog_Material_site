...
def pdf_repr(x):
    if x is True:
        ...
        return b"(" + x.replace(b"\\", b"\\\\").replace(b"(", b"\\(").replace(b")",
                                                                              b"\\)") + b")"  # XXX escape more chars? handle binary garbage
    else:
        return bytes(x)
...
class PdfParser:
    ...
    def read_pdf_info(self):
        ...
        check_format_condition(isinstance(self.root[b"Pages"], IndirectReference),
    ...
    re_trailer_end = re.compile(whitespace_mandatory + br"trailer" + whitespace_optional + br"\<\<(.*\>\>)" + newline
                                + br"startxref" + newline + br"([0-9]+)" + newline + br"%%EOF" + whitespace_optional + br"$",
                                re.DOTALL)
    re_trailer_prev = re.compile(whitespace_optional + br"trailer" + whitespace_optional + br"\<\<(.*?\>\>)" + newline
                                 + br"startxref" + newline + br"([0-9]+)" + newline + br"%%EOF" + whitespace_optional,
                                 re.DOTALL)
    ...
    def interpret_trailer(klass, trailer_data):
        ...
        while True:
            ...
            if not m:
                ...
                check_format_condition(m and m.end() == len(trailer_data),
            ...
            trailer[key] = value
        check_format_condition(b"Size" in trailer and isinstance(trailer[b"Size"], int),
                               "/Size not in trailer or not an integer")
        check_format_condition(b"Root" in trailer and isinstance(trailer[b"Root"], IndirectReference),
    ...
    re_indirect_reference = re.compile(
        whitespace_optional + br"([-+]?[0-9]+)" + whitespace_mandatory + br"([-+]?[0-9]+)" + whitespace_mandatory + br"R(?=" + delimiter_or_ws + br")")
    re_indirect_def_start = re.compile(
        whitespace_optional + br"([-+]?[0-9]+)" + whitespace_mandatory + br"([-+]?[0-9]+)" + whitespace_mandatory + br"obj(?=" + delimiter_or_ws + br")")
    ...
    re_xref_subsection_start = re.compile(
        whitespace_optional + br"([0-9]+)" + whitespace_mandatory + br"([0-9]+)" + whitespace_optional + newline_only)
        ...
        while True:
            ...
            for i in range(first_object, first_object + num_objects):
                ...
                if not is_free:
                    ...
                    check_format_condition(i not in self.xref_table or self.xref_table[i] == new_entry,
    ...
    def read_indirect(self, ref, max_nesting=-1):
        ...
        check_format_condition(generation == ref[1],
                               "expected to find generation %s for object ID %s in xref table, instead found generation %s at offset %s" \
                               % (ref[1], ref[0], generation, offset))
        value = self.get_value(self.buf, offset + self.start_offset, expect_indirect=IndirectReference(*ref),
