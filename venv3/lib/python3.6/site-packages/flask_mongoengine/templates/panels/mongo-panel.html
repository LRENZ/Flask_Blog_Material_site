...
<table class="mongo-op-table">
    ...
    <tr class="{{ loop.cycle('flDebugOdd','flDebugEven') }}{% if query.internal %} toolbar-query{% endif %}">
        ...
        <td><a href="javascript:void(0);" class="mongo-toggle-data" data-row="mongo-data-{{ loop.index }}">Toggle</a>
        </td>
        {% elif title == "Inserts" %}
        {% set colspan = 5 %}
        ...
        <td>{{ query.upsert }}</td>
        {% endif %}
        <td><a href="javascript:void(0);" class="mongo-toggle-trace"
               data-row="mongo-{{ title|lower }}-{{ loop.index }}">Toggle</a></td>
    </tr>
    {% if title == "Queries" %}
    ...
    {% endif %}
    <tr class="{{ loop.cycle('flDebugOdd','flDebugEven') }} mongo-stack-trace"
        id="mongo-{{  title|lower }}-{{ loop.index }}">
        <td colspan="{{ colspan}} ">
            <table>
                <thead>
                <tr>
