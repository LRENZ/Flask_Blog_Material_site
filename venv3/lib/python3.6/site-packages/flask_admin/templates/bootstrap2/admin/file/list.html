...
<ul class="breadcrumb">
    ...
    {% if breadcrumbs %}
    <li>
        <span class="divider">/</span><a href="{{ get_dir_url('.index_view', path=breadcrumbs[-1][1]) }}">{{
        breadcrumbs[-1][0] }}</a>
    </li>
    {% endif %}
</ul>
...
<div class="btn-toolbar">
    ...
    <div class="btn-group">
        ...
        {% else %}
        <a class="btn btn-large" href="{{ get_dir_url('.mkdir', path=dir_path) }}">{{ _gettext('Create Directory')
            }}</a>
        {%- endif -%}
    </div>
    {% endif %}
