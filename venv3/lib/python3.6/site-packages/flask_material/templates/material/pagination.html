...
<nav>
    <ul class="pagination{% if size %} pagination-{{size}}{% endif %}" {{kwargs|xmlattr}}>
        ...
        {% if not pagination.has_prev %} class="disabled"{% endif %}><a
            href="{{_arg_url_for(endpoint, url_args, page=pagination.prev_num) if pagination.has_prev else '#'}}">{{prev}}</li></a>
        {%- endif -%}
        ...
        {% if not pagination.has_next %} class="disabled"{% endif %}><a
            href="{{_arg_url_for(endpoint, url_args, page=pagination.next_num) if pagination.has_next else '#'}}">{{next}}</li></a>
        {%- endif -%}
    </ul>
</nav>
{% endwith %}
